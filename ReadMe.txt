...
Level.cpp : It contains all the logic for the game level. It contains the entites vector and all the functions related to the game.

LevelGraphics.cpp : This cpp files contains the render function for the game.
Menu.cpp : It is used to create the menu system for the game.
...
Menu.h : It defines the menu states and menu gui functions.
...
In Main.cpp, It creates different states of the window which can be seen in Menu.h file, It initializes raylib stuff, loads the resources from ResourceManager.h, spawn the player and starts the game loop when it is in GAME state. It runs the render and update function(Level.h). Later it unloads the resources(In ResourceManager.cpp before closing the window.
