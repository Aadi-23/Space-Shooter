...
Game.h : It defines the menu states and menu gui functions.
...
In Main.cpp, It creates different states of the window which can be seen in Game.h file, It initializes raylib stuff, loads the resources from ResourceManager.h, spawn the player and starts the game loop when it is in GAME state. It runs the render and update function(Level.h). Later it unloads the resources(In ResourceManager.cpp before closing the window.
